...
zeta = reshape(sqrt(((phi_n(3:end, 2:end-1) - phi_n(2:end-1,2:end-1)).^2/h^2 + (phi_n(2:end-1,3:end)-phi_n(2:end-1,1:end-2)).^2)/(2*h)^2),m^2,1);
xi = reshape(sqrt(((phi_n(2:end-1,3:end)-phi_n(2:end-1,2:end-1)).^2/h^2+(phi_n(3:end,2:end-1)-phi_n(1:end-2,2:end-1)).^2)/(2*h)^2),m^2,1);

% C,D,E are the entries of the matrix for the system to solve.
C = reshape(drac(phi_n(2:end-1,2:end-1),h),m^2,1)*mu./(h^2*xi); 
D = reshape(drac(phi_n(2:end-1,2:end-1),h),m^2,1)*mu./(h^2*zeta); 
E = -2*mu/h^2*reshape(drac(phi_n(2:end-1,2:end-1),h),m^2,1).*(1/zeta+1/xi)'-1/dt;
...
for k=0:m-1
    ...
    A(k+1,k+1) = A(k+1,k+1) + 4/3 * C((k+1)*m); % Buttom boundary:
...
opts.SYM = true;
phi_nn(2:end-1,2:end-1) = reshape(linsolve(full(A),b,opts),m,m);
